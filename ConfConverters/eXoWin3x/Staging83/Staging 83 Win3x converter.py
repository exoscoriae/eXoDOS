...
TEMPLATE_83_PATH = SCRIPT_DIR / "Staging 83 eXoWin3x Template.conf"
...
KEY_IGNORE = {
    ...
    ("render", "image_adjustments"), ("midi", "mididevice"),
}
...
SILENT_IGNORE_KEYS = {
    ...
    ("speaker", "lpt_dac"), ("sdl", "priority"),
    ("speaker", "tandy"), ("speaker", "disney"),
    ("speaker", "ps1audio"), ("render", "monochrome_palette"),
    ("dos", "pcjr_memory_config"), ("render", "monochrome_palette"),
    ("mouse", "mouse_multi_display_aware"), ("mouse", "mouse_middle_release"),
    ("dosbox", "vga_render_per_scanline"),
}
...
def convert_config_unified(conf_path: Path, source_label: str):
    ...
    for section in conf_src.sections():
        ...
        for key, value in conf_src[section].items():
            ...
            # --------------------------------------------------------------------
            # SPECIAL CASE: [sdl] sensitivity -> [mouse] mouse_sensitivity
            # --------------------------------------------------------------------
            if (
                (ssection == "sdl" and skey == "sensitivity")
                or
                (ssection == "mouse" and skey == "mouse_sensitivity")
                ):

                    tmpl_val = template83.get(
                        "mouse",
                        "mouse_sensitivity",
                        fallback=""
                    )

                    source_val = value.strip()

                    # 1. Ignore if it matches the template default (40)
                    if source_val == tmpl_val:
                        continue

                    # 2. Ignore known source defaults that should be replaced by template
                    if source_val in ("100", "100,100"):
                        ignored_diff.append(
                            ("mouse", "mouse_sensitivity", tmpl_val, source_val)
                        )
                        continue

                    # 3. Otherwise update mouse_sensitivity
                    update_template_key(
                        output_lines,
                        section_positions,
                        template83,
    ...
    print(f"\n{CYAN}===== 83.x TEMPLATE VALUES UPDATED FROM SOURCE ====={RESET}\n")
    if not changed:
    ...
    f"      require {RED}file_locking=off{RESET} to avoid 'Invalid projector file' error.\n"
    )
                ...
                if dosbox_path.exists():
                    ...
                    if entry_index is not None:
                        ...
                        if current_entry.lower() == desired.lower():
                            print(f"\n{GREEN}Dosbox3x.txt for this game is already set to Staging 83.0.{RESET}")
                        else:
                            print(f"\n{RED}Would you like to update dosbox3x.txt to use Staging 83.0 for this game? (Y/N){RESET}")
                            while True:
                                ch = msvcrt.getch().decode("utf-8").lower()
                                if ch in ("y", "n"):
                                    break
